...
class ChatApiTests(unittest.TestCase):
    ...
    def test_chat_message_returns_400_for_empty_message(self):
        response = self.client.post("/chat/message", json={"message": "   "})

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()["detail"], "El mensaje no puede estar vacío.")

    @patch("app.api.chat.generate_chat_reply", side_effect=RuntimeError("provider down"))
    def test_chat_message_returns_500_with_generic_message_on_ai_error(self, _mock_reply):
        response = self.client.post("/chat/message", json={"message": "Hola"})

        self.assertEqual(response.status_code, 500)
        self.assertEqual(response.json()["detail"], "No se pudo generar la respuesta en este momento.")
